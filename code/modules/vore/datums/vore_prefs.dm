...
/client/verb/open_vore_preferences()
	set category = "OOC"
	set name = "Vore Panel"
	set desc = "Open the vore panel."

	if (!prefs)
		return

	if (!prefs.vr_prefs)
		prefs.vr_prefs = new(src, prefs)
	prefs.vr_prefs.update_static_data(usr)
	prefs.vr_prefs.ui_interact(usr)
			...
			switch(action_choice)
				...
				if("Devour")
					if (target in inside.absorbed)
						...
						return
					user.Ingest(target, inside.owner, inside)
				else
					return
		if("eject_all")
			var/belly = params["belly"]
